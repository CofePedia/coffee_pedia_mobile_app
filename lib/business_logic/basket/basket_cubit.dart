...
class BasketCubit extends Cubit<BasketState> {
    ...
    basketRepository
      ...
      .then(
        (basketList) async {
          ...
          emit(LocalBasketLoaded(basketList),);
  ...
  Future addProductInLocalBasket(BasketLocal basketLocal) async {
    List<BasketLocal> baskets = await basketRepository.getAllLocalProductsFromBasket();
    ///////prints
    ...
    baskets.forEach((element) async {
      if(element.productId == basketLocal.productId){
        ...
        await basketRepository
            .updateQuantityInLocalBasket(basketLocal.productId!, basketLocal.quantity!)
            .then(
                (value) => emit(UpdateLocalBasket(),),
            )
            .catchError((error) {
    ...
    if(!itemFound){
      ...
      await basketRepository
          ...
          .then((value) => emit(UpdateLocalBasket(),),
      ).catchError((error) {
        print("local basket error $error");
  ...
  void deleteFromLocalBasket(int productId) async {
    await basketRepository
        .deleteFromLocalBasket(productId)
        .then(
          (value) async {
